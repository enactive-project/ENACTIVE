...
class Ltt_Semantic_NN(nn.Module):
    def __init__(self, ground_truth_dim, native_dim,
        ...
        self.ltt_head = nn.Linear(action_head_hidden_size, 2)
    ...
    def macro_forward(self, global_state, native_state, self_msl_token_state, bandit_msl_token_state,
                      hor_mask, ver_mask, shoot_mask, last_action_one_hot, deliberation_cost):
        # *** attention forward *** #
        ...
        last_action_embedding = self.activation(self.ltt_embedding(last_action_one_hot))
        steer_state = torch.cat((policy_state, last_action_embedding), dim=-1)
        ltt_hidden = self.activation(self.ltt_hidden(steer_state))
        ltt_exp = torch.exp(self.ltt_head(ltt_hidden))
        # ltt_prob = torch.sigmoid(ltt_head)
        # ltt_prob_ = ltt_prob + deliberation_cost.unsqueeze(-1)
        # ltt_prob_final = torch.clamp(ltt_prob_, 0, 1)
        lts_prob = ltt_exp / torch.sum(ltt_exp, dim=-1, keepdim=True)
        lts_prob += deliberation_cost
        lts_prob = lts_prob / torch.sum(lts_prob, dim=-1, keepdim=True)
        ...
        return hor_prob, ver_prob, shoot_prob, v_head, policy_state, lts_prob
    ...
    def select_action(self, global_state, native_state, self_msl_token_state, bandit_msl_token_state,
                      hor_mask: list, ver_mask: list, shoot_mask: list, last_action_one_hot: list, deliberation_cost):
        ...
        last_action_one_hot = torch.tensor(last_action_one_hot, dtype=torch.float32)
        ...
        hor_prob, ver_prob, shoot_prob, value, policy_state, ltt_prob = self.macro_forward(global_state,
                                                                                       native_state,
        ...
        return hor, ver, shoot, v_c, nn_c, ltt, value

    def batch_forward(self, global_state, native_state, self_msl_token_state, bandit_msl_token_state, hor_masks,
                      ver_masks, shoot_masks, hor_one_hots, ver_one_hots, last_action_one_hots, deliberation_cost):
        # function used for training #
        hor_probs, ver_probs, shoot_probs, v_head, policy_state, ltt_probs = self.macro_forward(global_state,
                                                                                                native_state,
                                                                                                self_msl_token_state,
                                                                                                bandit_msl_token_state,
                                                                                                hor_masks,
                                                                                                ver_masks,
                                                                                                shoot_masks,
                                                                                                last_action_one_hots,
                                                                                                deliberation_cost)
        v_c_probs, nn_c_probs = self.hybrid_forward(policy_state, hor_one_hots, ver_one_hots)

        return hor_probs, ver_probs, shoot_probs, v_c_probs, nn_c_probs, v_head, ltt_probs
    ...
    def get_log_prob_and_values(self, global_state, native_state, self_msl_token_state, bandit_msl_token_state,
                                ...
                                hor_one_hots, ver_one_hots, ltts, last_action_one_hots, deliberation_cost):

        hor_probs, ver_probs, shoot_probs, v_c_probs, nn_c_probs, value, ltt_probs = self.batch_forward(
            global_state, native_state, self_msl_token_state, bandit_msl_token_state, hor_masks, ver_masks, shoot_masks,
            hor_one_hots, ver_one_hots, last_action_one_hots, deliberation_cost)
        ...
        ltt = ltts.clone()
        ltt[torch.isnan(ltt)] = 0
        ltt_probs = ltt_probs.gather(-1, ltt.long())
        ltt_probs[torch.isnan(ltt)] = 1
        ...
        ans = torch.log(hor_probs + self.log_protect) + \
              ...
              torch.log(ltt_probs + self.log_protect)
