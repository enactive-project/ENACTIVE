from agents.state_machine_agent.machine_config import semantic_maneuver_default, top_tactics, macro_tactics, MagicNumber
from agents.state_machine_agent.combo import Abort, Evade, Intercept, Format, Banzai
...
class MachineBird(AgentBase):
    def __init__(self):
        self.side = None
        self.interval = 1
        self.maneuver_model = ["F22bot", "F22bot", "F22bot", "F22bot"]
        self.combo_executing = [None, None, None, None]
        self.threat_level = ["none", "none", "none", "none"]
    ...
    def after_reset(self, env, side):
        ...
        team2_y = env.state_interface["AMS"][self.side * env.red + 2]["Xg_1"]["value"]
        team3_y = env.state_interface["AMS"][self.side * env.red + 3]["Xg_1"]["value"]
        if not ((team2_y <= team3_y) ^ (bandit0_y <= bandit1_y)):
            self.default_target.append((1 - self.side) * env.red)
            self.default_target.append((1 - self.side) * env.red + 1)
        else:
            self.default_target.append((1 - self.side) * env.red + 1)
        ...
        team2_x = env.state_interface["AMS"][self.side * env.red]["Xg_0"]["value"]
        if team2_x >= 0:
            if team2_y >= team3_y:
                self.default_clockwise.append(-1)
                self.default_clockwise.append(1)
    ...
    def check_team_missile_work(self, env, team_idx: list) -> bool:
        for i in team_idx:
    ...
    def get_team_missile_work_target(self, env, team_idx: list) -> list:
        missile_target_list = []
        for i in team_idx:
            missile_set = env.state_interface["AMS"][i]["SMS"]
            for msl in missile_set:
    ...
    def get_escape_available_list(self, env, team_idx: list) -> list:
        escape_flag_list = []
        for i in team_idx:
    ...
    def check_alive_num(self, env, team_idx: list):
        team_alive_num = 0
        bandit_alive_num = 0
        for i in team_idx:
    ...
    """running script"""
    def rule_script(self, env, semantic_maneuver_list, team_idx: list):
        for j in range((1 - self.side) * env.red, env.red + (1 - self.side) * env.blue):
        ...
        select_top_tactics = self.top_rule(env, team_idx)
        if select_top_tactics == "all_defense":
            maneuver_list, shoot_list, target_list = self.all_defense_rule(env, semantic_maneuver_list, team_idx)
        elif select_top_tactics == "offense_and_defense":
            maneuver_list, shoot_list, target_list = self.offense_and_defense_rule(env, semantic_maneuver_list, team_idx)
        else:
            maneuver_list, shoot_list, target_list = self.all_offense_rule(env, semantic_maneuver_list, team_idx)

        return maneuver_list, shoot_list, target_list

    """top tactic selection"""
    def top_rule(self, env, team_idx: list):
        all_defense_flag = self.judge_all_defense(env, team_idx)
        offense_and_defense_flag = self.judge_offense_and_defense(env, team_idx)
        if all_defense_flag:
    ...
    def judge_all_defense(self, env, team_idx: list):
        flag = True
        for i in team_idx:
            alive = env.state_interface["AMS"][i]["alive"]["value"]
            if int(alive + 0.1) < 1:
    ...
    def judge_offense_and_defense(self, env, team_idx: list):
        team_alive_num, bandit_alive_num = self.check_alive_num(env, team_idx)
        escape_flag_list = self.get_escape_available_list(env, team_idx)
    ...
    """target assignment"""
    def target_assign(self, env, team_idx: list) -> list:
        target_list = []
        team_alive_num, bandit_alive_num = self.check_alive_num(env, team_idx)
        for i in team_idx:
    ...
    """three top tactics"""
    def all_defense_rule(self, env, semantic_maneuver_list, team_idx: list):
        target_list = self.target_assign(env, team_idx)
        macro_tactics_list, shoot_list = self.all_defense_macro_tactics_assign(env, team_idx)
        maneuver_list = []
        tactic_l = []
        ...
        for i in team_idx:
            ...
            rel_i = k - env.red if self.side else k
            alive = env.state_interface["AMS"][i]["alive"]["value"]
            if int(alive + 0.1) == 1:
                ...
                if self.combo_executing[rel_i] is not None:
                    macro_tactic = self.combo_executing[rel_i]
    ...
    def offense_and_defense_rule(self, env, semantic_maneuver_list, team_idx: list):
        target_list = self.target_assign(env, team_idx)
        macro_tactics_list, shoot_list = self.offense_and_defense_macro_tactics_assign(env, target_list, team_idx)
        maneuver_list = []
        tactic_l = []
    ...
    def all_offense_rule(self, env, semantic_maneuver_list, team_idx: list):
        target_list = self.target_assign(env, team_idx)
        macro_tactics_list, shoot_list = self.all_offense_macro_tactics_assign(env, target_list, team_idx)
        # shoot_available_list = self.shoot_available_assign(env, target_list)
        shoot_available_list = [True, True, True, True]
    ...
    """tactics assignment"""
    def all_defense_macro_tactics_assign(self, env, team_idx: list):
        macro_tactics_list = []
        shoot_list = []
        for i in team_idx:
            alive = env.state_interface["AMS"][i]["alive"]["value"]
    ...
    def offense_and_defense_macro_tactics_assign(self, env, target_list: list, team_idx: list):
        macro_tactics_list = []
        shoot_list = []
        ...
        for i in team_idx:
            ...
            if escape_flag:
                ...
                macro_tactic, shoot = self.general_offense_macro_tactic(env, i, target_id, team_idx)
            macro_tactics_list.append(macro_tactic)
            shoot_list.append(shoot)

        return macro_tactics_list, shoot_list

    def all_offense_macro_tactics_assign(self, env, target_list: list, team_idx: list):
        macro_tactics_list = []
        shoot_list = []
        team_alive_num, bandit_alive_num = self.check_alive_num(env, team_idx)
        if team_alive_num == 1 and bandit_alive_num == 1:  # 1v1 offense
            ...
            for i in team_idx:
                ...
                if int(alive + 0.1) == 1:
                    target_id = target_list[i - env.red if self.side else i]
                    macro_tactic, shoot = self.disadvantaged_offense_macro_tactic(env, i, target_id, team_idx)
                    macro_tactics_list.append(macro_tactic)
                    shoot_list.append(shoot)
                else:
                    macro_tactics_list.append(macro_tactics["other"]["maintain"])  # TODO
    ...
    def shoot_available_assign(self, env, target_list: list) -> list:
        shoot_available_flag_list = [True, True, True, True]
    ...
    def disadvantaged_offense_macro_tactic(self, env, self_id, target_id, team_idx: list):
        msl_remain = int(env.state_interface["AMS"][self_id]["AAM_remain"]["value"])
        if msl_remain <= 1:
            macro_tactic, shoot = self.general_offense_macro_tactic(env, self_id, target_id, team_idx)
        else:
    ...
    def general_offense_macro_tactic(self, env, self_id, target_id, team_idx: list):  # absolute index
        team_alive_num, bandit_alive_num = self.check_alive_num(env, team_idx)
        ...
        if msl_remain == 4:
            ...
            if bandit_alive_num == 1:
                ...
                team_msl_work_flag = self.check_team_missile_work(env, team_idx)
    ...
    def before_step_for_sample(self, env):
        semantic_maneuver_list = [deepcopy(semantic_maneuver_default), deepcopy(semantic_maneuver_default),
                                  deepcopy(semantic_maneuver_default), deepcopy(semantic_maneuver_default)]

        for i in range(2):
            team_idx = [self.side * env.red + i * 2, self.side * env.red + 1 + i * 2]
            maneuver_list, shoot_list, target_list = self.rule_script(env, semantic_maneuver_list, team_idx)
            # print(i, "m: ", maneuver_list, "s: ", shoot_list, "t: ", target_list)

            for k in team_idx:
                alive = env.state_interface["AMS"][k]["alive"]["value"]
                if int(alive + 0.1) == 1:
                    maneuver = maneuver_list[k - env.red - i * 2 if self.side else k - i * 2]
                    shoot = shoot_list[k - env.red - i * 2 if self.side else k - i * 2]
                    target = target_list[k - env.red - i * 2 if self.side else k - i * 2]
                    # print("step: ", maneuver)
                    # if i == 1:
                    #     target = 3
                    ...
                    env.action_interface["AMS"][k]["BotManeuver"]["base_direction"]["value"] = 10
                    env.action_interface["AMS"][k]["BotManeuver"]["combat_mode"]["value"] = 0
                    env.action_interface["AMS"][k]["BotManeuver"]["flag_after_burning"]["value"] = 1
                    env.action_interface["AMS"][k]["BotManeuver"]["horizontal_cmd"]["value"] = maneuver["horizontal_cmd"]
                    env.action_interface["AMS"][k]["BotManeuver"]["vertical_cmd"]["value"] = maneuver["vertical_cmd"]
                    env.action_interface["AMS"][k]["BotManeuver"]["vel_cmd"]["value"] = maneuver["vel_cmd"]
                    env.action_interface["AMS"][k]["BotManeuver"]["ny_cmd"]["value"] = maneuver["ny_cmd"]
                    env.action_interface["AMS"][k]["BotManeuver"]["clockwise_cmd"]["value"] = maneuver["clockwise_cmd"]
                    env.action_interface["AMS"][k]["BotManeuver"]["base_direction"]["value"] = maneuver["base_direction"]
                    env.action_interface["AMS"][k]["BotManeuver"]["maneuver_target"]["value"] = target
                    env.action_interface["AMS"][k]["action_target"]["value"] = target
                    # env.action_interface["AMS"][i]["action_shoot_target"]["value"] = -1
                    if not ((target < env.red) ^ (k < env.red)):
                        env.action_interface["AMS"][k]["action_shoot_target"]["value"] = -1
                    else:
                        if shoot:
                            env.action_interface["AMS"][k]["action_shoot_target"]["value"] = target - (1 - self.side) * env.red
                        else:
                            env.action_interface["AMS"][k]["action_shoot_target"]["value"] = -1
...
if __name__ == "__main__":
    ...
    from agents.state_machine_agent.simple_agent import Simple
