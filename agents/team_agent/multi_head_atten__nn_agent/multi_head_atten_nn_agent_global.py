...
from models.multi_head_atten_nn_global import MultiHead_Attention_NN as MHDPA_global
...
class MHDPA_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None):
        self.batchs = dict(state_global=[], state_native=[], state_token=[], maneuver=[], shot=[], target=[], mask=[],
    ...
    def _create_model(self):
        ...
        model = MHDPA_global(len(red_state_global), len(red_atten_state[0]),
                             len(red_atten_state[1][0]), len(red_atten_state[1]),
                             action_dims=model_action_dims, aircraft_num=Config.env.red)
    ...
    def before_step_for_sample(self, env):
        ...
        if self.target_type == "origin" or self.target_type == "without_self":
            maneuver_list, shots_list, targets_list = model.select_action(self.states_global, self.states_native, self.states_tokens,
                                                                          maneuver_masks, shot_masks, target_masks)

        elif self.target_type == "only_enemy":
            maneuver_list, shots_list, targets_list = model.select_action_after_target(self.states_global, self.states_native, self.states_tokens,
                                                                                       maneuver_masks, shot_masks, target_masks)
    ...
    def train(self, batchs):
        ...
        batch = dict(state_global=[], state_native=[], state_token=[], maneuver=[], shot=[], target=[], mask=[],
                     maneuver_masks=[], target_masks=[], shot_masks=[],
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                maneuver_prob, shoot_prob, target_prob, _ = model(states_global_b, states_native_b, states_token_b)
                # print(maneuver_prob)
                # print(shoot_prob)
                # print(target_prob)
