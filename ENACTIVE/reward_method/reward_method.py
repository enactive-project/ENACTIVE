...
def get_mean_team_reward_without_ith_aircraft(env: BattleSpace, ith: int, k:int, rewards_hyperparam_dict):
    sum_reward = 0
    other_agents_num = 0
    for i in range(k * env.red, env.red + k * env.blue):
        if i != ith:
            sum_reward += get_ith_aircraft_reward(env, i, rewards_hyperparam_dict)
            other_agents_num += 1
...
def get_ith_aircraft_reward(env: BattleSpace, i: int, rewards_hyperparam_dict):
    ...
    ret = ret + accumulate_fire_reward * (missile_launch * (missile_launch + 1)) / 2  # -  missile value increase with AMS remain
...
def get_solo_reward_range(env: BattleSpace, i: int, rewards_hyperparam_dict: dict):
