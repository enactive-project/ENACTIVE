...
from agents.single_agent.ltt.semantic_nn import Semantic_NN
from agents.single_agent.ltt.ltt_nn import Ltt_NN
...
class Ltt_Semantic_Agent(AgentBase):
    ...
    def _create_model(self):
        ...
        model = []
        model_major = Semantic_NN(self.state_dims['global_dim'], self.state_dims['native_dim'],
        ...
        model_ltt = Ltt_NN(self.state_dims['global_dim'], self.state_dims['native_dim'],
                           self.state_dims['self_msl_token_dim'], self.state_dims['self_msl_token_num'],
                           self.state_dims['bandit_msl_token_dim'], self.state_dims['bandit_msl_token_num'],
                           action_dims=self.model_action_dims)
    ...
    def get_delib_cost(self):
        ...
        if self.version < 50:
            self.deliberation_cost = 0
        elif self.version <= 500:
            self.deliberation_cost = -0.2 + (self.version - 50) * 0.2 / 450
        else:
            self.deliberation_cost = 0
    ...
    def before_step_for_sample(self, env):
        ...
        hor, ver, shoot, v_c, nn_c = model[0].select_action(self.states_global, self.states_native, msl_token_self,
                                                            msl_token_bandit, hor_masks, ver_masks, shoot_masks)

        if self.step == 0:
            self.last_hor = deepcopy(hor.tolist())
            self.last_ver = deepcopy(ver.tolist())
            self.last_v = deepcopy(v_c.tolist())
            self.last_nn = deepcopy(nn_c.tolist())

        # maneuver_num = hor_mask_len * ver_mask_len
        # maneuver_idx = ver_mask_len * self.last_hor[0] + self.last_ver[0]
        # last_maneuver_one_hot = index_to_one_hot(maneuver_idx, maneuver_num)
        # self.maneuver_one_hot = deepcopy(last_maneuver_one_hot)
        # last_maneuver_one_hot = torch.tensor(last_maneuver_one_hot).detach()
        ...
        last_action_one_hot = last_hor_one_hot + last_ver_one_hot + last_v_one_hot + last_nn_one_hot
        self.last_action_one_hot = deepcopy(last_action_one_hot)
        last_action_one_hot = torch.tensor(last_action_one_hot).detach()

        ltt = model[1].select_action(self.states_global, self.states_native, self.self_msl_tokens, self.bandit_msl_tokens,
                                     last_action_one_hot, self.deliberation_cost)
    ...
    def train(self, batchs):
        ...
        model[1].to(device)
        with torch.no_grad():
            fixed_log_probs, values = model[0].get_log_prob_and_values(states_global, states_native, self_msl_token,
                                                                       bandit_msl_token,
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                log_probs, values = model[0].get_log_prob_and_values(states_global_b, states_native_b,
                ...
                ltt_probs, _ = model[1].get_log_prob_and_values(states_global_b, states_native_b,
                                                                self_msl_token_b, bandit_msl_token_b,
                                                                ltts_b, last_action_one_hots_b, deliberation_cost_b)
                ...
                hor_prob, ver_prob, shoot_prob, v_c_prob, nn_c_prob, _ = model[0].batch_forward(states_global_b,
                                                                                                states_native_b,
                                                                                                self_msl_token_b,
                                                                                                bandit_msl_token_b,
                                                                                                hor_masks_b,
                ...
                # ltt loss #
                loss_ltt = (ltt_probs.squeeze(-1) * (advantages_b + Config.delib_cost)).mean()  # sigmoid
                ...
                loss_step_origin(model[0], loss_major)
                loss_step_origin(model[1], loss_ltt)
