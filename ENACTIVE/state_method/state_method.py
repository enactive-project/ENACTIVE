...
def get_kteam_aircraft_state_for_attention(env: BattleSpace, k: int, zerolize_invalid_token=True):
    ...
    for i in range(k * env.red, env.red + k * env.blue):
        ...
        ret_native.append(env.normalize(aircraft_i["AAM_remain"]) if aircraft_available else 0.0)
        # ret_native.append(env.normalize(aircraft_i["FCR_locked"]))
        ret_native.append(env.normalize(aircraft_i["RWR_nailed"]) if aircraft_available else 0.0)
        ret_native.append(env.normalize(aircraft_i["RWR_spiked"]) if aircraft_available else 0.0)
        ret_native.append(env.normalize(aircraft_i["TAS"]) if aircraft_available else 0.0)
        ret_native.append(env.normalize(aircraft_i["h_dot"]) if aircraft_available else 0.0)
        ret_native.append(env.normalize(aircraft_i["residual_chi"]) if aircraft_available else 0.0)
        ret_native.append(env.normalize(aircraft_i["Xg_2"]) if aircraft_available else 0.0)
        for j in aircraft_i["FCSModel"]:
            ret_native.append(env.normalize(j["FCS_available"]) if aircraft_available else 0.0)
        for j in aircraft_i["RWRModel"]:
            ret_native.append(env.normalize(j["RWR_fetched"]) if aircraft_available else 0.0)
        for j in aircraft_i["RadarModel"]:
            ret_native.append(env.normalize(j["FCR_locked"]) if aircraft_available else 0.0)
        for m in aircraft_i["SMS"]:
            for j in m["FCSGuide"]:
                ret_native.append(env.normalize(j["fcs_guide_info"]))
        for m in aircraft_i["SMS"]:
            for j in m["RadarGuide"]:
                ret_native.append(env.normalize(j["radar_guide_info"]))
        for m in aircraft_i["SMS"]:
            ret_native.append(env.normalize(m["state"]))
            ret_native.append(env.normalize(m["target_index"]))
            ret_native.append(1.0 - env.normalize(m["TGO"]))
            # add msl relative states #
            if int(m["state"]["value"] + 0.1) == 2 or int(m["state"]["value"] + 0.1) == 3:  # missile flying
                ret_native.append(env.normalize(m["r_dot_m"]))
                ret_native.append(env.normalize(m["TAS_m"]))
                ret_native.append(env.normalize(m["TA_m"]))
                ret_native.append(env.normalize(m["AO_m"]))
            else:
                ret_native.append(0)  # missle mounting or escaping #
                ret_native.append(0)
                ret_native.append(0)
                ret_native.append(0)
        ...
        for enemy_id in range(len(aircraft_i["attack_zone_list"])):
            if get_aircraft_available(env, enemy_id + (1 - k) * env.red) and get_aircraft_available(env, i):
                ...
                ret_native.append(env.normalize(dlz_list["Raero"]))
                ret_native.append(env.normalize(dlz_list["Rmax"]))
                ret_native.append(env.normalize(dlz_list["Rmin"]))
                ret_native.append(env.normalize(dlz_list["Ropt"]))
                ret_native.append(env.normalize(dlz_list["Rpi"]))
                ret_native.append(env.normalize(dlz_list["Rtr"]))
            else:
                ret_native.append(0)
                ret_native.append(0)
                ret_native.append(0)
        ...
        # add attitude angle
        ret_native.append(env.normalize(aircraft_i["attg_0"]))
        ret_native.append(env.normalize(aircraft_i["attg_1"]))
        # add overload
        ret_native.append(env.normalize(aircraft_i["n_y"]))
        # add out boarder time
        ret_native.append(env.normalize(aircraft_i["out_of_border_time"]))
        ret_native.append(env.normalize(aircraft_i["out_of_border_distance"]))

    # add agents' alive info for native
    for i in range(env.red + env.blue):
        aircraft_id = (i + env.red * k) % (env.red + env.blue)
        ret_native.append(env.normalize(env.state_interface["AMS"][aircraft_id]["alive"]))
    ...
    for j in range(env.blue + env.red):
        for t in range(j + 1, env.red + env.blue):
            temp_2 = []
            env.to_list(temp_2, env.state_interface["AMS"][(k * env.red + j) % (env.red + env.blue)][
                "relative_observation"][(k * env.red + t) % (env.red + env.blue)])
            temp_2 = copy.deepcopy(temp_2)
            if not (get_aircraft_available(env, (k * env.red + j) % (env.red + env.blue)) and get_aircraft_available(env, (k * env.red + t) % (env.red + env.blue))):
                for item in temp_2:
                    item["value"] = item["min"]
            for item in temp_2:
                temp.append(item)
    for item in temp:
        ret_native.append(env.normalize(item))
